...
import type { SeniorSquadApplication, HighPerformanceSquadApplication, JuniorAcademyApplication } from "@shared/schema";
...
function JuniorAcademyDialog({
  ...
  application: JuniorAcademyApplication | null;
  open: boolean;
  onClose: () => void;
  onUpdate: (status: string, notes: string) => void;
  isPending: boolean;
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            <span className="text-sm font-normal text-gray-500">— Junior Academy</span>
          </DialogTitle>
        </DialogHeader>

        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4 bg-gray-50 rounded-lg p-4">
            {application.athleteDob && (
              <div className="flex items-center gap-2 text-sm">
                <Calendar className="h-4 w-4 text-gray-400" />
                <span>DOB: {application.athleteDob}</span>
              </div>
            )}
          ...
          <div className="bg-blue-50 rounded-lg p-4">
            <p className="text-xs font-semibold text-blue-700 uppercase tracking-wide mb-2">Parent / Guardian</p>
            <p className="text-sm font-medium">{application.parentName}</p>
            <p className="text-sm text-gray-600">{application.parentEmail}</p>
            <p className="text-sm text-gray-600">{application.parentPhone}</p>
          </div>

          <div>
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3 flex items-center gap-1">
              <Trophy className="h-3 w-3" /> Sporting Background
            </p>
            <DetailRow label="Sports Played" value={application.sports} />
            <DetailRow label="Days Per Week Active" value={application.activityDays} />
            <DetailRow label="Favourite Sport" value={application.favSport} />
          </div>

          <div>
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3 flex items-center gap-1">
              <Target className="h-3 w-3" /> Goals &amp; Motivations
            </p>
            <DetailRow label="Parent Goals" value={application.parentGoals} />
            <DetailRow label="Athlete Goal" value={application.athleteGoal} />
            <DetailRow label="Nervous About" value={application.nervous} />
          </div>

          <div>
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3 flex items-center gap-1">
              <Clock className="h-3 w-3" /> Availability &amp; Logistics
            </p>
            <DetailRow label="Available Days" value={application.availDays} />
            <DetailRow label="Other Commitments" value={application.commitments} />
            <DetailRow label="Training Facilities" value={application.facilities} />
          </div>

          {(application.medical || application.injuries) && (
            <div>
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Health &amp; Medical</p>
              <DetailRow label="Medical Conditions" value={application.medical} />
              <DetailRow label="Injuries / History" value={application.injuries} />
            </div>
          )}

          <div>
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Preferences</p>
            <DetailRow label="Contact Preference" value={application.contactPref} />
            <DetailRow label="Feedback Preference" value={application.feedbackPref} />
            <DetailRow label="Photo Consent" value={application.photoConsent} />
            <DetailRow label="Coach Notes" value={application.coachNotes} />
          </div>
...
function JuniorApplicationTable({
  applications,
  onView,
}: {
  applications: JuniorAcademyApplication[];
  onView: (app: JuniorAcademyApplication) => void;
}) {
  if (applications.length === 0) {
    return (
      <div className="text-center py-16 text-gray-400">
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b text-xs text-gray-500 uppercase tracking-wide">
            ...
            <th className="text-left py-3 px-4 font-semibold">Parent Contact</th>
            <th className="text-left py-3 px-4 font-semibold">Submitted</th>
            <th className="text-left py-3 px-4 font-semibold">Status</th>
            <th className="text-left py-3 px-4 font-semibold">Action</th>
          </tr>
        ...
        <tbody>
          {applications.map((app) => (
            <tr key={app.id} className="border-b hover:bg-gray-50 transition-colors">
              <td className="py-3 px-4 font-medium">{app.athleteName}</td>
              <td className="py-3 px-4 text-gray-600 text-xs">{app.programme || "—"}</td>
              <td className="py-3 px-4 text-gray-600">
                <div>{app.parentName}</div>
                <div className="text-xs text-gray-400">{app.parentEmail}</div>
              </td>
              <td className="py-3 px-4 text-gray-500">
                {app.createdAt ? format(new Date(app.createdAt), "dd MMM yyyy") : "—"}
              </td>
...
export default function AdminApplications() {
  ...
  const [selectedJaApp, setSelectedJaApp] = useState<JuniorAcademyApplication | null>(null);
  const [activeProgram, setActiveProgram] = useState<"senior-squad" | "high-performance-squad">("senior-squad");
  const [dialogOpen, setDialogOpen] = useState(false);
  const [jaDialogOpen, setJaDialogOpen] = useState(false);
  ...
  const { data: jaApps = [], isLoading: jaLoading } = useQuery<JuniorAcademyApplication[]>({
    queryKey: ["/api/applications/junior-academy"],
    enabled: user?.role === "admin",
  });
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["/api/applications/junior-academy"] });
      toast({ title: "Decision saved", description: "Application status has been updated." });
      setDialogOpen(false);
      setJaDialogOpen(false);
    },
    onError: () => {
  ...
  const handleJaUpdate = (status: string, reviewNotes: string) => {
    if (!selectedJaApp) return;
    updateMutation.mutate({ program: "junior-academy", id: selectedJaApp.id, status, reviewNotes });
  };

  const pendingSenior = seniorApps.filter((a) => a.status === "pending").length;
  const pendingHp = hpApps.filter((a) => a.status === "pending").length;
  const pendingJa = jaApps.filter((a) => a.status === "pending").length;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
          ...
          <Card>
            <CardContent className="pt-5 pb-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-gray-500">Junior Academy</p>
                  <p className="text-2xl font-bold text-gray-900">{jaApps.length}</p>
                  <p className="text-xs text-gray-400">{pendingJa} pending review</p>
                </div>
                <User className="h-8 w-8 text-green-500 opacity-60" />
              </div>
            </CardContent>
          </Card>
          <Card>
        ...
        <Card>
          <CardHeader className="pb-0">
            <Tabs defaultValue="junior-academy">
              <TabsList>
                <TabsTrigger value="junior-academy">
                  Junior Academy
                  {pendingJa > 0 && (
                    <Badge className="ml-2 bg-yellow-100 text-yellow-800 text-xs font-semibold">{pendingJa}</Badge>
                  )}
                </TabsTrigger>
              ...
              <TabsContent value="junior-academy" className="mt-0">
                {jaLoading ? (
                  <div className="py-12 text-center text-gray-400 text-sm">Loading applications...</div>
                ) : (
                  <JuniorApplicationTable
                    applications={jaApps}
                    onView={(app) => { setSelectedJaApp(app); setJaDialogOpen(true); }}
                  />
                )}
              </TabsContent>
      ...
      <JuniorAcademyDialog
        application={selectedJaApp}
        open={jaDialogOpen}
        onClose={() => setJaDialogOpen(false)}
        onUpdate={handleJaUpdate}
        isPending={updateMutation.isPending}
      />
    </div>
  );
